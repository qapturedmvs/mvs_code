						...
						<div ng-switch-when='0'>
							...
							<div seen-id="{{item.feed_id}}" class="feedHolder seen" ng-if="item.feed_type == 'sn'">
								...
								<div class="feedContent {{item.seen_type}}" ng-if="item.seen_type == 'movie_group_seen'">
									...
									<div class="feedInfo">
										<div class="textContent">
											<div class="text"><a href="<?php echo $site_url; ?>user/wall/actions/{{item.usr_nick}}">{{item.usr_name}}</a> has marked <span ng-repeat="grp in item.grp" ng-if="$index < 2"><a class="movieName" href="<?php echo $site_url; ?>movie/{{grp.mvs_slug}}">{{grp.mvs_title}}</a><span ng-if="$first">, </span></span> and <span ng-repeat="grp in item.grp" ng-if="$last">{{$index-1}}</span> other movies as seen.</div>
										</div>
										<div class="movieGroup" rel="{{item.grp.length}}">
											<a ng-repeat="grp in item.grp" ng-if="$index >= 2" class="moviePoster lazy" data-original="<?php echo $site_url; ?>{{grp.mvs_poster}}" href="<?php echo $site_url; ?>movie/{{grp.mvs_slug}}" title="{{grp.mvs_title}}"></a>
											<hr class="qFixer" />
										</div>
										<div class="time"><span title="{{item.feed_time}}">{{item.feed_ago}}</span></div>
										<hr class="qFixer" />
