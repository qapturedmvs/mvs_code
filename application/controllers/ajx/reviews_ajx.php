<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	class Reviews_Ajx extends Ajx_Controller{
		...
		public function add_comment($reply = NULL){
			...
			if($this->logged_in){
				...
				if($vars){
					...
					$data = array('usr_id' => $this->user['usr_id'], 'act_ref_id' => (isset($vars['ref'])) ? $vars['ref'] : 0, 'act_type' => (isset($vars['type'])) ? $vars['type'] : 0, 'id' => (isset($vars['id'])) ? $vars['id'] : 0, 'act_text' => $vars['text'], 'act_spl_fl' => $vars['spl']);
